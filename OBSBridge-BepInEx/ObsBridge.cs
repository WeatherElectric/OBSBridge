...
public static class ObsBridge
{
    ...
    private static void ObsConnected(object sender, EventArgs e)
    {
        ...
        Plugin.Logger.LogMessage("Connected to OBS!");
    }
    
    private static void ObsDisconnected(object sender, ObsDisconnectionInfo e)
    {
        Connected = false;
        OnDisconnected?.Invoke(sender, e);
        Plugin.Logger.LogMessage("Disconnected from OBS!");
    }
    ...
    {
        Plugin.Logger.LogMessage("Attempting to connect to OBS...");
        try
        {
            Obs.ConnectAsync(Plugin.Preferences.WebsocketURL.Value, Plugin.Preferences.WebsocketPassword.Value);
        }
        catch (AuthFailureException e)
        {
            Plugin.Logger.LogError($"Failed to authenticate with OBS. Error: {e.Message}");
        }
    ...
    {
        Plugin.Logger.LogMessage("Disconnecting from OBS...");
        Obs.Disconnect();
        Connected = false;
    }
    ...
    /// <summary>
    /// Called when the connection to OBS is lost.
    /// </summary>
    public static Action<object, ObsDisconnectionInfo> OnDisconnected;
    ...
    public static void StartRecording()
    {
        ...
        Plugin.Logger.LogDebug("Starting recording...");
        try
        {
            Obs.StartRecord();
        }
        ...
        {
            Plugin.Logger.LogError($"Failed to start recording. Error: {e.Message}");
        }
        catch (Exception e)
        {
            Plugin.Logger.LogError($"Failed to start recording. Error: {e.Message}");
    ...
    public static void PauseRecording()
    {
        ...
        Plugin.Logger.LogDebug("Pausing recording...");
        try
        {
            Obs.PauseRecord();
        }
        ...
        {
            Plugin.Logger.LogError($"Failed to pause recording. Error: {e.Message}");
        }
        catch (Exception e)
        {
            Plugin.Logger.LogError($"Failed to pause recording. Error: {e.Message}");
    ...
    {
        if (!RecordPaused) return;
        Plugin.Logger.LogDebug("Resuming recording...");
        try
        {
            Obs.ResumeRecord();
        }
        ...
        {
            Plugin.Logger.LogError($"Failed to resume recording. Error: {e.Message}");
        }
        catch (Exception e)
        {
            Plugin.Logger.LogError($"Failed to resume recording. Error: {e.Message}");
    ...
    {
        if (!RecordActive) return;
        Plugin.Logger.LogDebug("Stopping recording...");
        try
        {
            Obs.StopRecord();
        }
        ...
        {
            Plugin.Logger.LogError($"Failed to stop recording. Error: {e.Message}");
        }
        catch (Exception e)
        {
            Plugin.Logger.LogError($"Failed to stop recording. Error: {e.Message}");
    ...
    public static void StartStreaming()
    {
        ...
        Plugin.Logger.LogDebug("Starting stream...");
        try
        {
            Obs.StartStream();
        }
        ...
        {
            Plugin.Logger.LogError($"Failed to start streaming. Error: {e.Message}");
        }
        catch (Exception e)
        {
            Plugin.Logger.LogError($"Failed to start streaming. Error: {e.Message}");
    ...
    {
        if (!StreamActive) return;
        Plugin.Logger.LogDebug("Stopping stream...");
        try
        {
            Obs.StopStream();
        }
        ...
        {
            Plugin.Logger.LogError($"Failed to stop streaming. Error: {e.Message}");
        }
        catch (Exception e)
        {
            Plugin.Logger.LogError($"Failed to stop streaming. Error: {e.Message}");
    ...
    public static void StartReplayBuffer()
    {
        ...
        Plugin.Logger.LogDebug("Starting replay buffer...");
        try
        {
        ...
        {
            Plugin.Logger.LogError($"Failed to start replay buffer. Error: {e.Message}");
        }
        catch (Exception e)
        {
            Plugin.Logger.LogError($"Failed to start replay buffer. Error: {e.Message}");
    ...
    {
        if (!ReplayBufferActive) return;
        Plugin.Logger.LogDebug("Stopping replay buffer...");
        try
        {
        ...
        {
            Plugin.Logger.LogError($"Failed to stop replay buffer. Error: {e.Message}");
        }
        catch (Exception e)
        {
            Plugin.Logger.LogError($"Failed to stop replay buffer. Error: {e.Message}");
    ...
    {
        if (!ReplayBufferActive) return;
        Plugin.Logger.LogDebug("Saving replay buffer...");
        try
        {
        ...
        {
            Plugin.Logger.LogError($"Failed to save replay buffer. Error: {e.Message}");
        }
        catch (Exception e)
        {
            Plugin.Logger.LogError($"Failed to save replay buffer. Error: {e.Message}");
